...
from Simulation import SensorManager
...
class Simulator:
    def __init__(self, host='127.0.0.1', port=2000, set_autopilot=True,
                 window_w=1024, window_h=1024, enable_ld=False, enable_od=False):
        ...
        self.sensor_list = []
        if enable_ld:
            self.add_lane_detection()
        if enable_od:
            self.add_obstacle_detection()
    ...
    def add_lane_detection(self):
        print("Initializing Lane Detection")
        cg = CameraGeometry()
        ld = LaneDetector(cam_geom=cg)

        print("\t Adding Camera Sensor")
        camera = SensorManager.SensorManager(self.world, self.display_manager, 'RGBCamera',
    ...
    def add_obstacle_detection(self):
        print("Initializing Obstacle Detection")
        print("\t Adding Lidar Sensor")
        lidar = SensorManager.SensorManager(self.world, self.display_manager, 'SemanticLiDAR',
                                            carla.Transform(carla.Location(x=0, z=2.4)), self.vehicle,
                                            {'channels': '64', 'range': '100', 'points_per_second': '100000',
                                             'rotation_frequency': '20'})

        visualization_cam = SensorManager.SensorManager(self.world, self.display_manager, 'RGBCamera',
        ...
        def obstacle_detection_visualizer(image):
            ...
            if visualization_cam.display_man.render_enabled():
        ...
        self.sensor_list.extend([lidar, visualization_cam])
